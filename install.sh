...
modify_port() {
    ...
    echo "当前端口: ${cur_port:-未设置 (默认 8484)}"
...
create_systemd_service() {
    ...
    if [[ -n "$PREFIX" && "$PREFIX" == *"/com.termux"* ]]; then
        echo "错误：Termux 环境不支持 systemd 服务"
        return 1
    fi
    
    if [ "$EUID" -ne 0 ] && ! sudo -n true 2>/dev/null; then
        echo "错误：需要 root 权限或 sudo 权限来创建 systemd 服务"
        return 1
    fi
    
    if [ ! -x "${SCRIPT_DIR}/clewdr/clewdr" ]; then
        echo "错误：ClewdR 未安装，请先安装 ClewdR"
        return 1
    fi
...
Description=ClewdR - Claude API Proxy
Documentation=https://github.com/Xerxes-2/clewdr
...
User=$current_user
Group=$current_group
WorkingDirectory=${SCRIPT_DIR}/clewdr
ExecStart=${SCRIPT_DIR}/clewdr/clewdr
Restart=always
RestartSec=10
